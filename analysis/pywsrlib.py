...
def aws_key(s, suffix=""):
    ...
    path = "%4d/%02d/%02d/%s" % (s["year"], s["month"], s["day"], s["station"])

    name = "%s%04d%02d%02d_%02d%02d%02d" % (
        s["station"],
        s["year"],
        s["month"],
        s["day"],
...
def read_nexrad_archive_s3(name):
    bucket = "noaa-nexrad-level2"
    ...
    with tempfile.NamedTemporaryFile() as temp:
        ...
        radar = pyart.io.read_nexrad_archive(temp.name)
...
def z_to_refl(z, wavelength=0.1071):
    ...
    log_eta = (
        np.log10(z)
        + 5 * np.log10(np.pi)
        + np.log10(K_m_squared)
        - 4 * np.log10(wavelength)
    )
...
def get_sweeps(radar, field):
    ...
    for i, tilt in enumerate(unique_tilts):
        ...
        elif field in ["velocity", "spectrum_width"]:
...
def get_volumes(radar, field="reflectivity"):
    """
    ...
    for j, sweep in enumerate(sweeps):
        ...
        AZ[j], RNG[j] = np.meshgrid(sweep["az"], sweep["rng"], indexing="ij")
        DATA[j] = sweep["data"]
        ELEV[j] = np.full_like(DATA[j], sweep["elev"].reshape(-1, 1))
...
def radarInterpolant(data, az, rng, method="nearest"):
    ...
    data = np.vstack((data[-1, :], data, data[0, :]))
    ...
    return RegularGridInterpolator(
        (az, rng), data, method=method, bounds_error=False, fill_value=np.nan
    )
...
def radar2mat(
    ...
    if fields is None:
        ...
        if not (np.all(available)):
            warnings.warn(
                "requested fields %s were not available" % (fields[available != True])
            )
    ...
    if sweeps is not None:
        ...
        elev2ind = interp1d(
            available_elevs, inds, kind="nearest", fill_value="extrapolate"
        )
